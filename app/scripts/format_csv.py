...
from app.utils.file_utils import FILES_DIR
...
def main():
    ...
    for filepath in csv_files:
        if format_csv_file(filepath):
            print(f"{Fore.GREEN}[OK] Formatted: {filepath.name}{Style.RESET_ALL}")
            changed += 1
        else:
            print(f"{Fore.YELLOW}[SKIP] No change: {filepath.name}{Style.RESET_ALL}")
    if changed:
        print(
