...
def main():
    ...
    if py_files:
        ...
        if fix:
            ...
            _run(
                "isort (format)",
                ["python", "-m", "app.scripts.format_imports"] + py_files,
            )
        else:
            ...
            rc = _run(
                "isort (check)",
                ["python", "-m", "app.scripts.check_imports"] + py_files,
            )
            if rc != 0:
                exit_code = 1
    ...
    if csv_files:
        if fix:
            _run(
                "format_csv (format)",
                ["python", "-m", "app.scripts.format_csv"] + csv_files,
            )
        else:
            rc = _run(
                "lint_csv (check)", ["python", "-m", "app.scripts.lint_csv"] + csv_files
            )
            if rc != 0:
                exit_code = 1
    ...
    print()
    if exit_code == 0:
        print(f"{Fore.GREEN}{Style.BRIGHT}[OK] All checks passed!{Style.RESET_ALL}")
    else:
        print(
            f"{Fore.RED}{Style.BRIGHT}[FAIL] Some checks failed. "
